...
def make_rotation_lightcurve_frame(t, **kwargs):
    ...
    img, mesh = cf.projection_2d(band, direction='+x', stellar_radius=params['RadiusStar'], interpolator=interpolator,
                                                    angle=(0,np.deg2rad(angle)), image_radius=20, pixel_count=100, var_list=v, use_simple_g=False)
    vmax = np.nanmax(img)
    ...
    light_curve =np.load(f'Light_curve_{name}.npy')
...
def make_rotation_frame(t):
    ...
    ax.clear()
    img, mesh = cf.projection_2d(band, direction='+x', stellar_radius=params['RadiusStar'], interpolator=interpolator,
                                                    angle=(np.deg2rad(angle), np.deg2rad(30)), image_radius=20, pixel_count=100, var_list=v, use_simple_g=False)
    vmax = np.nanmax(img)
    norm = LogNorm(vmax=vmax, vmin=vmax/1e6 , clip=True)
...
if __name__ == '__main__':
    ...
    rotation_movie = VideoClip(make_rotation_frame, duration=10)
    rotation_movie.write_gif(f'Random_rotation_{name}.gif', fps=24)